...
Profile content is centralized in `src/config/profile.ts`. Site metadata and
external links live in `src/config/site.ts`. Keep reusable presentation in
`src/components` and avoid moving future business logic into this app.
